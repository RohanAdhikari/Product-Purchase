...
from config import CATEGORY_DICT, RETURN_POLICY_DICT, username_input, password_input
...
# Sidebar Admin Toggle (always visible)
...
# --- Admin Login check for viewing admin page ---
if st.session_state.view_history and not st.session_state.admin_logged_in:
    st.subheader("🔐 Admin Login Required")
    input_username = st.text_input("Username", key="admin_username_main")
    input_password = st.text_input("Password", type="password", key="admin_password_main")
    if st.button("Login", key="admin_login_btn_main"):
        if input_username == username_input and input_password == password_input:
...
# Show Admin Page if toggled and logged in
if st.session_state.view_history and st.session_state.admin_logged_in:
